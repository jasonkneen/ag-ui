---
title: "State"
description: "The shared state and the conversation itself: replaced wholesale, amended by patch — draft"
---
...
[snapshot–delta pattern](/spec/draft/basic/patterns/snapshots); this page adds
...
the [pattern](/spec/draft/basic/patterns/snapshots) says about patches —
...
- A consumer-held message *absent* from the snapshot is dropped — the producer
  ...
  cannot know about: [activity](/spec/draft/events/activity) messages, which
...
[`StateSnapshotEvent`](/spec/draft/schema#statesnapshotevent), [`StateDeltaEvent`](/spec/draft/schema#statedeltaevent) and [`MessagesSnapshotEvent`](/spec/draft/schema#messagessnapshotevent) are defined
by the [schema reference](/spec/draft/schema), with [`State`](/spec/draft/schema#state) deliberately
unconstrained — any JSON value, not only an object — and deltas as
[`JsonPatch`](/spec/draft/schema#jsonpatch).

## Error Handling

A structurally malformed patch is fatal; a well-formed patch that fails to
apply is warned about and skipped, per the
[pattern](/spec/draft/basic/patterns/snapshots#when-a-patch-does-not-apply).
