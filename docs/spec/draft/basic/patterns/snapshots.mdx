...
description: "How evolving values travel: replacement wholesale, amendment by JSON Patch — draft"
---
...
[state](/spec/draft/events/state) (`STATE_SNAPSHOT`, `STATE_DELTA`, and
`MESSAGES_SNAPSHOT` for the conversation) and
[activity](/spec/draft/events/activity) (`ACTIVITY_SNAPSHOT`,
...
([activity](/spec/draft/events/activity)'s `replace: false` is the one case).
...
- A delta applies against the consumer's current value: the last snapshot as
  ...
  [carried none](/spec/draft/basic/run-input#state).
...
- Patch operations are deliberately open objects: the schema marks them so, and
  ...
  [processing model](/spec/draft/basic/processing) MUST NOT strip.
...
[processing model](/spec/draft/basic/processing)'s list rule applies: the
...
The patch shape — [`JsonPatch`](/spec/draft/schema#jsonpatch), its operations, [`JsonPointer`](/spec/draft/schema#jsonpointer) — is defined by
the [schema reference](/spec/draft/schema), by reference to RFC 6902.
