...
interface HttpAgentConfig extends AgentConfig {
  ...
  endpoints?: HttpAgentEndpoints // Override specific endpoint URLs
}

interface HttpAgentEndpoints {
  capabilities?: string // Override the capabilities discovery endpoint
}
```
...
Discovers the remote agent's capabilities by making a `GET` request to
`{url}/capabilities` (or the configured `capabilitiesUrl`).

```typescript
async getCapabilities(): Promise<AgentCapabilities>
```

See [Capabilities](/concepts/capabilities) for usage patterns and the full type
definition.
...
- `endpoints`: Optional overrides for specific endpoint URLs (e.g., `capabilities`)
