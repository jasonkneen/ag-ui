...
source omits `mimeType`, `UriContent` infers it from the URL.

Object-shaped `metadata` properties are copied to the MEAI content's
`AdditionalProperties`. Other JSON metadata values are preserved under the
`metadata` key. For inline data, a string `filename` metadata property is also
assigned to `DataContent.Name` so file-capable providers receive the filename.

In the client direction, `AsAGUIMessages()` maps `DataContent` and `UriContent`
