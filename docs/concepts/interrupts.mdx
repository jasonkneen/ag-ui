...
| `frontend_tool_call` | Agent is paused waiting for the client to run a frontend tool it has already emitted. | `toolCallId` must be set. |
...
`frontend_tool_call` makes an otherwise invisible pause visible, so a client can
tell "finished" from "waiting on me" and can persist or cancel the wait. It is
the one core reason a client may answer without `resume[]`: sending the ordinary
`ToolMessage` for that `toolCallId` is an equivalent answer, and the agent
translates it into the same response. Clients that enforce "every pending
interrupt must be covered by `resume[]`" must treat a matching tool result as
coverage.
...
| AWS Strands | `@ag-ui/aws-strands` / `ag-ui-strands` | ✅ Forwards native Strands interrupts via `RunFinishedEvent.outcome`. In Python, a frontend tool the agent is waiting on is also reported this way, with `reason: "frontend_tool_call"` and the emitted `toolCallId`; it is answered either by `resume[]` or by the client's ordinary `ToolMessage` for that tool call. |
