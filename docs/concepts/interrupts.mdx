...
| --- | --- | --- |
| LangGraph | `@ag-ui/langgraph` / `ag-ui-langgraph` | ✅ Emits `RunFinishedEvent.outcome = {type:"interrupt"}`, accepts `RunAgentInput.resume[]`. Legacy `CustomEvent(name="on_interrupt")` still emitted by default; disable via `enableLegacyOnInterruptEvent: false`. Subclass hooks available for custom HITL translation. |
