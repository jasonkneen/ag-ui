...
| `subagentRunId` | The subagent whose work raised this interrupt — absent for a root-raised one. Attribution lives per interrupt because one run can carry interrupts from several subagents; the matching `SubagentFinished` closes with `outcome: "suspended"` (see [Subagents](/concepts/subagents#suspension)). |
