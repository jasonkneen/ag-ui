...
`HttpAgent` implements `getCapabilities()` by making a `GET` request to a
...
// Fetches GET https://my-agent.example.com/api/capabilities
const capabilities = await agent.getCapabilities()
```

The endpoint defaults to `{url}/capabilities` but can be overridden via
`endpoints`:
...
const agent = new HttpAgent({
  url: "https://my-agent.example.com/api",
  endpoints: {
    capabilities: "https://my-agent.example.com/api/v2/caps",
  },
})
```

Remote agents should expose a `GET /capabilities` endpoint that returns the
