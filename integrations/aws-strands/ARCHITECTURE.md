...
- **Opt-in cross-origin access** (`createStrandsApp`): omitting `corsOrigin` installs no CORS middleware at all, so cross-origin access is a deliberate choice rather than the starting position. This is a live divergence, not a port gap, and the default is the whole of it: Python's `create_strands_app` still defaults to wildcard-open. Unless `cors_enabled=False` is passed it adds `CORSMiddleware` with `allow_origins=origins or ["*"]`, so it falls back to the wildcard even for `origins=[]` (an empty list is falsy, so `origins or ["*"]` selects the wildcard), and it warns about that implicit fallback with a `FutureWarning` rather than refusing it. The two adapters agree on the wildcard: Python computes `allow_credentials=bool(origins) and not is_wildcard`, and TypeScript's `policyAllowsCredentials` derives the same rule from the resolved origin, so neither pairs credentials with a wildcard and neither offers a way to override the derivation. TypeScript then refuses two cases Python still allows, both of them the origin `null`, which a browser sends from a sandboxed iframe or a `file://` page and which belongs to no site: a policy naming nothing but `"null"` carries no credentials, and no request whose own `Origin` is `null` carries them either, whatever admitted it. The second half is why `cors` is constructed with an options delegate rather than a fixed object: the decision is per request, so a list holding `"null"` beside real origins keeps credentials for those origins instead of losing them policy-wide. Starlette takes one `allow_credentials` for the whole policy, so the Python side would need a different shape to match. TypeScript reaches that rule through `normalizeCorsOrigin` first, which collapses any array containing `"*"` to the bare string, so `["*"]` and `["*", "https://app.tld"]` are allow-all rather than allowlists.
