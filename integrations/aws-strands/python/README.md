...
| Wrapper/agent recreation in the same process or host                          | Can resume through `StrandsAgentConfig.session_manager_provider` only when the returned `SessionManager` implementation actually restores the Strands checkpoint and metadata for that thread/session. Implementing the `SessionManager` interface alone does not guarantee restoration.                                                                                                                       |
| Cross-process or replica resume                                               | Requires stable shared thread/session identity and a durable shared session backend accessible to every process or replica. The provider must return managers connected to that same durable session; merely passing an arbitrary per-thread `SessionManager` instance is not sufficient.                                                                                                                      |
