...
export class SubgraphsPage {
  ...
  constructor(page: Page) {
    ...
    this.flightOptions = page.locator('.flight-option, [data-testid*="flight"]');
    this.klmFlightOption = page.locator('.flight-option', { hasText: 'KLM' });
    this.unitedFlightOption = page.locator('.flight-option', { hasText: 'United' });

    this.hotelOptions = page.locator('.hotel-option, [data-testid*="hotel"]');
    this.hotelZephyrOption = page.locator('.hotel-option', { hasText: 'Hotel Zephyr' });
    this.ritzCarltonOption = page.locator('.hotel-option', { hasText: 'Ritz-Carlton' });
    this.hotelZoeOption = page.locator('.hotel-option', { hasText: 'Hotel Zoe' });
  ...
  async waitForSupervisorCoordination() {
    await expect(
      this.page.getByText(/supervisor|coordinate|specialist|routing|Amsterdam|San Francisco/i).first()
    ).toBeVisible();
  }
}
