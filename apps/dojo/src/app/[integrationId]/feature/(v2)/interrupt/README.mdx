...
This is wired for **Mastra (Local)**. Mastra's tool `suspend()` is the native
HITL primitive; the `@ag-ui/mastra` adapter bridges `tool-call-suspended` to
`on_interrupt`. The interrupt value keeps Mastra's own shape — the renderer
reads `event.value.suspendPayload`. Resume requires a local agent (remote
